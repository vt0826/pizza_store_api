...
handlers.checkUsers = function(data, callback) {
  var acceptableMethods = ['post', 'get', 'put', 'delete'];
  if (acceptableMethods.indexOf(data.method) > -1) {
    handlers._checkUsers[data.method](data, callback);
  } else {
    callback(405);
  }
};
...
handlers._checkUsers = {};

//checkUsers-get
//required data: email
//optional data: none
handlers._checkUsers.get = function(data, callback) {
...
//campaigns
handlers.campaigns = function(data, callback) {
  var acceptableMethods = ['post', 'get', 'put', 'delete'];
  if (acceptableMethods.indexOf(data.method) > -1) {
    handlers._campaigns[data.method](data, callback);
  } else {
    callback(405);
  }
};

//container for all the campaign methods
handlers._campaigns = {};

//campaigns - index get
//required data :email
//optonal data: none
handlers._campaigns.get = function(data, callback) {
  ...
  var email =
    ...
    data.payload.email.trim().length > 0
      ...
      : false;
  //if (email) {
  //lookup the campaigns and return the list of campaigns
  _data.list('campaigns', function(err, campaignsData) {
    if (!err && campaignsData) {
      callback(200, campaignsData);
    } else {
      callback(404);
    }
  });
...
//campaign
handlers.campaign = function(data, callback) {
  var acceptableMethods = ['post', 'get', 'put', 'delete'];
  if (acceptableMethods.indexOf(data.method) > -1) {
    handlers._campaign[data.method](data, callback);
  } else {
    callback(405);
  }
};

//container for message submethods
handlers._campaign = {};
...
handlers._campaign.post = function(data, callback) {
  //check taht all required fileds are filled out
  var title =
  ...
  if (title && description && email) {
    //make sure the user does not already exist
    _data.read('campaigns', title, function(err, data) {
      if (err) {
        //create user object
        var campaignObject = {
          title: title,
        ...
        // create the new user
        _data.create('campaigns', title, campaignObject, function(err) {
          if (!err) {
            callback(200);
          } else {
            callback(500, {Error: 'Could not create the new campaign'});
          }
        });
      } else {
        //user already exist
        callback(400, {Error: 'that campaign already exist'});
      }
    });
  } else {
    callback(400, {Error: 'Missing required fields'});
...
//required data :title
//optonal data: none
handlers._campaign.get = function(data, callback) {
  var title =
    ...
    data.queryStringObject.title.trim().length > 0
      ...
      : false;
  if (title) {
    _data.read('campaigns', title, function(err, messageData) {
      if (!err) {
        callback(200, messageData);
      } else {
        callback(404);
      }
    });
...
//required data: title
//optional data : none
handlers._campaign.put = function(data, callback) {
  var title =
    typeof data.queryStringObject.title == 'string' &&
    data.queryStringObject.title.trim().length > 0
      ? data.queryStringObject.title.trim()
  ...
  var member =
    typeof data.payload.member == 'string' &&
    data.payload.member.trim().length > 0
      ? data.payload.member.trim()
      : false;

  if (member) {
    //look up user to see if they do exsit
    _data.read('users', member, function(err, userData) {
      if (!err && userData) {
        // check if theres such user in the system
        _data.read('campaigns', title, function(err, campaignData) {
          if (!err && campaignData) {
            // make member isnt already in the list
            // store the new updates
            if (!campaignData.members.includes(member)) {
              campaignData.members.push(member);
              _data.update('campaigns', title, campaignData, function(err) {
                if (!err) {
                  userData.campaigns.push(title);
                  _data.update('users', member, userData, function(err) {
                    if (!err) {
                      callback(200);
                    } else {
                      callback(500, {
                        Error: 'could not update the users campaigns list',
                      });
                    }
                  });
                } else {
                  callback(500, {
                    Error: 'could not update the campaigns memebr list',
                  });
                }
              });
            } else {
              callback(400, {Error: 'member already exist'});
            }
          } else {
            callback(400, {Error: 'specified message does not exist'});
          }
        });
      } else {
